...
int main(int argc, char** argv)
{
    try
    {
        ...
        ws.handshake(host, "/auth?FF=abcd");
        ...
        req.id = 10;
        req.content = "good good bang bang bang x 100";
        std::string package;
        MCodec::serial(req, package);

        // Send the message
        ws.write(net::buffer(package));
...
//         This buffer will hold the incoming message
        ...
        std::string binary = beast::buffers_to_string(buffer.data());
        std::cout << MCodec::debugShowHex(binary, "resp") <<std::endl;
