...
std::shared_ptr<WebsocketSession> SessionManager::getSession(const std::string& deviceId)
{
...
int SessionManager::addSession(const std::string& deviceId, WebsocketSession* session)
{
	std::lock_guard<std::mutex> lock(_mutex);
	log2I(IceClient::getInstance()->getCommunicator(), "deviceId=" << deviceId);
	auto r = _sessions.insert(std::pair<std::string, WebsocketSession*>(deviceId, session));
	if(false == r.second)
	{
		return -1;
	}
