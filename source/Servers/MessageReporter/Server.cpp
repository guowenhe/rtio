...
public:
    ...
    virtual int run(int argc, char*[]) override
    {
        try
        {
            ...
            auto config = std::make_shared<MessageReporterConfig>();
            ...
            auto workers = std::make_shared<MessageReporterWorkers>(workerNum);
            ...
            adapter->add(std::make_shared<MessageReporterI>(workers), Ice::stringToIdentity(properties->getProperty("Identity")));
            adapter->activate();
