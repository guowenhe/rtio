...
void HttpSession::response(std::shared_ptr<HttpRespInfo> info, std::shared_ptr<GlobalResources::SendReturn> data)
{
    ...
    if(http::status::ok == status)
    {
        if(data)
        {
            ...
            j["device_code"] = data->deviceCode;
            j["device_message"] = data->deviceMessage;
            resp->body() = j.dump();
        }
    }
    else
...
void HttpSession::handleRequest(http::request<Body, http::basic_fields<Allocator>>&& req)
{
    ...
    try
    {
        ...
        info->para.deviceId = checkReqirdString(reqJson, "device_id", 64);
        info->para.message = checkReqirdString(reqJson, "message", 512);

        auto optionTimeLive = checkOptionInt(reqJson, "time_live");
        if(optionTimeLive.first)
        {
